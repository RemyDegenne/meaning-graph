...
drag in that tool's build (Verso, SubVerso, MD4Lean, …).
...
[[require]]
name = "MeaningGraph"
git = "https://github.com/RemyDegenne/meaning-graph"
rev = "main"
```
...
- `lake build MeaningGraphTest` — `#guard` unit checks of the name classification, the `Expr`-level
  constant collection, the notation and coercion recoveries, and the graph passes.
